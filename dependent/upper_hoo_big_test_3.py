...
for round in range(1, int(n+1)):
    ...
    for i in [0,1,2,3]:
        players[i].sort()
        if players[i].count('1_king_black') == 1 and i==0:
            if players[i].count('2_airplane_black') == 1:
                if players[i].count('3_eleplant_black') == 1:
                    if players[i].count('1_king_red') == 0:
    ...
    if have_hoo :
        ...
        for j in range(0,4):
            ...
            if players[j].count('1_king_red') == 1:
                if players[j].count('2_airplane_red') >= 1:
                    if players[j].count('3_eleplant_red') >= 1:
                        sum += 1
