    ...
    for player in players:
        if players.index(player) != 0 :
            ...
            if set(['1_king_red','2_airplane_red','3_eleplant_red']).issubset(player) :
                if player.count('2_airplane_red') == 2 or player.count('3_eleplant_red') == 2 :
                    return 1

            if set(['1_king_black','2_airplane_black','3_eleplant_black']).issubset(player) :
                if player.count('2_airplane_black') == 2 or player.count('3_eleplant_black') == 2 :
                    return 1

            if set(['4_boat_red','5_horse_red','6_biggun_red']).issubset(player) :
                if player.count('4_boat_red') == 2 or player.count('5_horse_red') == 2 or player.count('6_biggun_red') == 2 :
                    return 1
    return 0
