...
for round in range(1, int(n+1)):
        ...
        if players[i].count('4_boat_black') == 2:
            if players[i].count('5_horse_black') == 2:
                if players[i].count('6_biggun_black') == 2:
                    if players[i].count('4_boat_red') == 0:
                        if players[i].count('5_horse_red') == 0 :
                            if players[i].count('6_biggun_red') == 0:
                                have_hoo = True
    ...
    if have_hoo :
        ...
        for j in range(0,4):
            if i == j:
                continue
            if players[j].count('4_boat_red') == 2:
                if players[j].count('5_horse_red') == 2:
                    if players[j].count('6_biggun_red') == 2:
                        sum += 1
