...
for round in range(1, int(n+1)):
    ...
    have_jook = False
    jook_big_have_index = -1
    for i in range(0,4):
        players[i].sort()
        #print players[i]
        if players[i].count('7_soldier_black') == 4:
            if players[i].count('7_soldier_red') <= 1:
                have_jook = True
                dependent_sample_space += 1
                jook_big_have_index = i
    ...
    print '===================================================================================================================================='
    if have_jook :
        i = jook_big_have_index
        for j in range(0,4):
            if i == j:
                continue
            if players[j].count('7_soldier_red') >= 4:
                sum += 1
